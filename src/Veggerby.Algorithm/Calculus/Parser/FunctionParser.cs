...
namespace Veggerby.Algorithm.Calculus.Parser
{
    ...
    {
        private static readonly Regex _number = new Regex(@"^(?<number>([\-+]?[0-9]+(\.[0-9]+)?((e|E)[\-+]?[0-9]+)?|pi|π))", RegexOptions.IgnoreCase);
        private static readonly Regex _operation = new Regex(@"^(?<operation>\+|-|\*|\/|\^)", RegexOptions.IgnoreCase);
        private static readonly Regex _function = new Regex(@"^(?<function>sin|cos|tan|exp|ln|log|log2|sqrt|root|min|max|!)", RegexOptions.IgnoreCase);
        private static readonly Regex _variable = new Regex(@"^(?<variable>[a-z][a-z0-9]*)", RegexOptions.IgnoreCase);
