...
{
    private readonly IDictionary<string, UnspecifiedConstant> _constants = new Dictionary<string, UnspecifiedConstant>();
    ...
    private Operand Compile(BinaryNode node)
    {
        ...
        {
            case "+": return left is not null && right is not null ? Addition.Create(left, right) : left ?? right;
            case "-": return left is not null && right is not null ? Subtraction.Create(left, right) : (left is null ? Negative.Create(right) : left);
